...
public class SecureServerNamespace extends ManagedNamespace {
	...
	public SecureServerNamespace(OpcUaServer server, String namespaceUri) {
		...
		// Dynamic Rand 100
		{
			...
			rand100Node.getFilterChain().addLast(
					...
					AttributeFilters.getValue(
							ctx ->
							new DataValue(new Variant(random.nextInt(100))) //Setup rand value between 0 - 100 on each read
							)
					);
